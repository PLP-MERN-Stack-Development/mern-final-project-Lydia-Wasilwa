  ...
  return (
    <form onSubmit={submit} className="bg-white shadow-md p-6 rounded-lg w-full max-w-md mx-auto">
      ...
      <input className="w-full p-2 border rounded mb-3" placeholder="Email"
        type="email" onChange={(e) => setForm({ ...form, email: e.target.value })} />

      <input className="w-full p-2 border rounded mb-3" placeholder="Password"
        type="password" onChange={(e) => setForm({ ...form, password: e.target.value })} />

      <button className="w-full bg-blue-600 text-white p-2 rounded" onClick={submit}>
        Register
      </button>
    </form>
  );
